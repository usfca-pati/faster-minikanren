...
;; (require "weak-imm-map.rkt")
;; (define empty-intmap (empty-wi-map))
;; (define (intmap-count w) (wi-map-count w))
;; (define (intmap-ref w k)
;;   (wi-map-ref w k (lambda () unbound)))
;; (define (intmap-set w k v) (wi-map-set w k v))

; (define empty-intmap (empty-my-dict))
; (define (intmap-count w) (dict-count w))
; (define (intmap-ref w k)
;   (dict-ref w k (lambda () unbound)))
; (define (intmap-set w k v) (dict-set w k v))

(require "hamt/hamt.rkt")
(define empty-intmap empty-hamt)
(define (intmap-count w) (hamt-count w))
(define (intmap-ref w k)
  (hamt-ref w k (lambda () unbound)))
(define (intmap-set w k v) (hamt-set w k v))

; (define empty-intmap '())
; (define (intmap-count w) (length w))
; (define (intmap-ref w k)
;   (let ([p (assq k w)])
;     (if (pair? p) (second p) unbound)))
; (define (intmap-set w k v) (cons (list k v) w))
...
(define (set-c st x c)
  (state-with-C
    st
    (intmap-set (state-C st) x c)))

(define (lookup-c st x)
  (let ((res (intmap-ref (state-C st) x)))
    (if (unbound? res)
      empty-c
      res)))
...
(define (remove-c x st)
  (state-with-C st (intmap-set (state-C st) x empty-c)))
...
(define-syntax conda
  (syntax-rules ()
    ...
    ((_ (g0 g ...) (g1 g^ ...) ...)
     (lambda (st)
       ;; resume all suspensions for the first goal
        ;; on failure, move to (g1 g^ ...) ...
        ;; on success, commit to the first choice
        (define (force-cut stream)
          (case-inf
           stream
           (() ((conda (g1 g^ ...) ...) st))
           ((f) (force-cut (f)))
